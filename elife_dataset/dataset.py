...
class ElifeDataset(Dataset):
    def __init__(self, data, tokenizer, prompt_template: str = "{article}"):
        self.data = data
...
def get_basic_datasets(dataset, tokenizer) -> tuple[ElifeDataset, ElifeDataset]:
    train_dataset = ElifeDataset(dataset['train'], tokenizer)
    eval_dataset = ElifeDataset(dataset['validation'], tokenizer)
